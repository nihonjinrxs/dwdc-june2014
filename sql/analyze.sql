...
with all_visits (first_name, visit_date, height) as (
  select first_name, visit_date, height from dwdc.visits1
  union all
  select first_name, visit_date, height from dwdc.visits2
  union all
  select first_name, visit_date, height from dwdc.visits3
  union all
  select first_name, visit_date, height from dwdc.visits4
)
, all_visits_and_ids (id, first_name, dob, visit_date, height) as (
  select i.id, i.first_name, i.dob, v.visit_date, v.height
  from dwdc.ids i left join all_visits v on i.first_name = v.first_name
)
